//! Golden negotiated room-operation envelopes (client issue #128).
#![allow(
    clippy::expect_used,
    clippy::panic,
    clippy::unwrap_used,
...
use signal_fish_client::protocol::{
    ClientMessage, LobbyState, ReconnectedPayload, ReplayStatus, RoomJoinedPayload,
...
fn human_readable_room_operation_id_requires_canonical_text_and_echoes_exactly() {
    ...
    for (description, noncanonical) in cases {
        ...
        let result_json = format!(
            r#"{{"type":"RoomOperationResult","data":{{"operation_id":"{noncanonical}","result":{{"type":"RoomLeft"}}}}}}"#
        );
        serde_json::from_str::<ServerMessage>(&result_json)
            .expect_err("noncanonical echoed room operation id must be rejected");
    }

    for invalid in ["null", r#""not-a-uuid""#] {
        let json = format!(
            r#"{{"type":"RoomOperation","data":{{"operation_id":{invalid},"operation":{{"type":"LeaveRoom"}}}}}}"#
        );
        serde_json::from_str::<ClientMessage>(&json)
            .expect_err("invalid room operation id must be rejected");
    }
}

#[test]
fn room_operation_debug_is_variant_only_and_redacts_nested_secrets() {
    let request = RoomOperationRequest::Reconnect {
        player_id: PLAYER_ID,
        room_id: ROOM_ID,
        auth_token: "request-secret".to_string(),
    };
    assert_eq!(format!("{request:?}"), "Reconnect");

    let result = RoomOperationResult::Reconnected(Box::new(reconnected()));
    let debug = format!("{result:?}");
    assert_eq!(debug, "Reconnected");
    assert!(!debug.contains("next-token"));

    let client = ClientMessage::RoomOperation {
        operation_id: OPERATION_ID,
        operation: Box::new(request),
    };
    assert_eq!(format!("{client:?}"), "RoomOperation");
    let server = ServerMessage::RoomOperationResult {
        operation_id: OPERATION_ID,
        result: Box::new(result),
    };
    assert_eq!(format!("{server:?}"), "RoomOperationResult");
}
