...
signal-fish-client = { version = "0.14.0", default-features = false, features = ["polling-client"] }
signal-fish-client-godot = "0.14.0"
```
...
of client release 0.14.0; a versioned
`signal-fish-client = "0.14.0"` dependency is sufficient.
...
run against server 0.9.1; a second clean job pins server 0.4.0 as the explicit
