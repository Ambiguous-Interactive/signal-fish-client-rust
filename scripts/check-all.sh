...
#   bash scripts/check-all.sh --quick  # run only phases 1-4 (fmt, ffi, clippy, test)
#
# Phases:
#   1. cargo fmt            (required)
#   2. FFI safety           (delegates to scripts/check-ffi-safety.sh)
#   3. cargo clippy          (required)
#   4. cargo test            (required)
#   5. cargo doc             (required)
#   6. cargo deny            (optional — skip if not installed)
#   7. cargo audit           (optional — skip if not installed)
#   8. Panic-free policy     (delegates to scripts/check-no-panics.sh)
...
PHASE_NAMES[2]="FFI safety"
PHASE_NAMES[3]="cargo clippy"
PHASE_NAMES[4]="cargo test"
PHASE_NAMES[5]="cargo doc"
PHASE_NAMES[6]="cargo deny"
PHASE_NAMES[7]="cargo audit"
PHASE_NAMES[8]="Panic-free policy"
PHASE_NAMES[9]="Docs validation"
PHASE_NAMES[10]="Unused deps (cargo-machete)"
PHASE_NAMES[11]="Workflow lint"
PHASE_NAMES[12]="Publish dry-run"
PHASE_NAMES[13]="Examples validation"
PHASE_NAMES[14]="Semver API compatibility"
PHASE_NAMES[15]="Miri (UB detection)"
...
if [ "$QUICK" = true ]; then
    echo -e "${YELLOW}Mode: --quick (phases 1-4 only)${NC}"
fi
echo ""
...
# ── Phase 2: FFI safety ─────────────────────────────────────────────
echo -e "${YELLOW}Phase 2/$TOTAL_PHASES: FFI safety check (scripts/check-ffi-safety.sh)...${NC}"
if [ -f "$SCRIPT_DIR/check-ffi-safety.sh" ]; then
    if bash "$SCRIPT_DIR/check-ffi-safety.sh" 2>&1; then
    ...
    echo -e "${YELLOW}SKIP: scripts/check-ffi-safety.sh not found.${NC}"
    PHASE_RESULTS[2]="SKIP"
fi
echo ""

# ── Phase 3: cargo clippy ──────────────────────────────────────────
echo -e "${YELLOW}Phase 3/$TOTAL_PHASES: Running Clippy (cargo clippy — 3 feature combos)...${NC}"
PHASE3_FAIL=false
...
# ── Phase 4: cargo test ────────────────────────────────────────────
echo -e "${YELLOW}Phase 4/$TOTAL_PHASES: Running tests (cargo test — 3 feature combos)...${NC}"
PHASE4_FAIL=false
for feature_flags in "" "--all-features" "--no-default-features"; do
    label="${feature_flags:-default features}"
...
# ── Phase 5: cargo doc ─────────────────────────────────────────────
echo -e "${YELLOW}Phase 5/$TOTAL_PHASES: Building documentation (cargo doc)...${NC}"
...
# ── Phase 6: cargo deny ────────────────────────────────────────────
echo -e "${YELLOW}Phase 6/$TOTAL_PHASES: Checking dependency policy (cargo deny)...${NC}"
...
# ── Phase 7: cargo audit ───────────────────────────────────────────
echo -e "${YELLOW}Phase 7/$TOTAL_PHASES: Running security audit (cargo audit)...${NC}"
...
# ── Phase 8: Panic-free policy ──────────────────────────────────────
echo -e "${YELLOW}Phase 8/$TOTAL_PHASES: Panic-free policy check...${NC}"
...
echo -e "${YELLOW}Phase 9/$TOTAL_PHASES: Docs validation (markdownlint, lychee, typos)...${NC}"
PHASE9_FAILURES=0
PHASE9_RAN=0
...
if ! command -v markdownlint-cli2 &>/dev/null && ! command -v markdownlint &>/dev/null; then
    ...
    if $MDL_CMD "**/*.md" 2>&1; then
        ...
        PHASE9_FAILURES=$((PHASE9_FAILURES + 1))
    fi
fi
...
if [ "$PHASE9_FAILURES" -gt 0 ]; then
    echo -e "${RED}Phase 9: FAIL ($PHASE9_FAILURES sub-check(s) failed)${NC}"
    mark_phase_fail 9
elif [ "$PHASE9_RAN" -eq 0 ]; then
    echo -e "${YELLOW}Phase 9: SKIP (no docs validation tools installed)${NC}"
...
# ── Phase 10: Unused deps (cargo-machete) ───────────────────────────
echo -e "${YELLOW}Phase 10/$TOTAL_PHASES: Checking for unused dependencies (cargo-machete)...${NC}"
...
# ── Phase 11: Workflow lint ─────────────────────────────────────────
echo -e "${YELLOW}Phase 11/$TOTAL_PHASES: Workflow lint (actionlint, yamllint, shellcheck)...${NC}"
...
# 13c: extract-rust-snippets.sh (optional — skip if script not found)
...
if [ "$PHASE13_FAILURES" -gt 0 ]; then
    echo -e "${RED}Phase 13: FAIL ($PHASE13_FAILURES sub-check(s) failed)${NC}"
    mark_phase_fail 13
else
...
# ── Phase 14: Semver API compatibility ────────────────────────────
echo -e "${YELLOW}Phase 14/$TOTAL_PHASES: Checking semver API compatibility (cargo-semver-checks)...${NC}"
...
# ── Phase 16: Fuzz smoke tests ───────────────────────────────────
# Runs each fuzz target for 10 seconds locally (shorter than CI's 30s).
# Requires nightly + cargo-fuzz.
echo -e "${YELLOW}Phase 16/$TOTAL_PHASES: Fuzz smoke tests (nightly, 10s per target)...${NC}"
...
echo -e "${YELLOW}Phase 18/$TOTAL_PHASES: Code coverage (cargo-llvm-cov)...${NC}"
if ! command -v cargo-llvm-cov &>/dev/null; then
    ...
    PHASE_RESULTS[18]="SKIP"
else
    if cargo llvm-cov --all-features --summary-only 2>&1; then
        echo -e "${GREEN}Phase 18: PASS${NC}"
        PHASE_RESULTS[18]="PASS"
    else
