...
guarantees documented below stay current. The published `0.14.0` dependency
with its [versioned API docs](https://docs.rs/signal-fish-client/0.14.0/)
...
| `tokio-runtime` | Yes (via `transport-websocket`) | Enables `tokio/rt`, `tokio/time`, `tokio/sync`, and `tokio/macros` for background task spawning | No | No |
