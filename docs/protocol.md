...
| `connected_at` | `String` | ISO 8601 timestamp of when the player connected. Empty when the server omits the field: protocol-v3 room snapshots trim it for privacy (signal-fish-server #539). A decoded snapshot whose field is empty re-serializes without the key, matching the trimmed v3 shape. |
...
| `connected_at` | `String` | ISO 8601 timestamp of when the spectator joined. Empty when the server omits the field: protocol-v3 spectator snapshots trim it for privacy (see `PlayerInfo`; signal-fish-server #539). |
...
| `ClientMessage::JoinRoom` / `JoinAsSpectator` (and their negotiated forms) | `password` | Join password for password-protected rooms (upstream access-control tier). Required for a sealed room; a password presented to an open room is refused (`PASSWORD_REQUIRED`). Omitted when unset, so existing joins keep byte-identical wire behavior; set it with `JoinRoomParams::with_password` for players and `join_as_spectator_with_password` for spectators. |
