...
- Return `Pending` without taking the frame when the transport has not accepted
  it.
- Once the transport calls `frame.take()`, it owns the frame and must retain all
  state needed to finish that exact send across later polls.
- After taking a frame, return `Ready` only when the frame was fully flushed or
  the send failed.
...
3. Remember that a send is in flight.
4. Drive `poll_flush` until ready, then clear the in-flight state.
