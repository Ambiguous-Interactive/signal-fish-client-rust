...
- **Version:** 0.14.0 lockstep across both crates
- **Edition:** 2021
...
GitHub hides ruleset bypass actors from workflow tokens, so maintainers verify an empty bypass list in the ruleset UI. Prepare Release uses only `GITHUB_TOKEN` and dispatches required workflows explicitly on its generated branch because token-created pull-request events are suppressed; path filters must not suppress a configured gate. No checked-in `GITHUB_TOKEN` workflow may auto-merge Dependabot PRs: such merges rely on drifting rules and can suppress push CI on the resulting SHA, so the fail-closed policy rejects Dependabot-specific workflows, automated-merge primitives, and write permissions outside the release/docs allowlist. Ruleset #14801090 live-enforces all required checks with no required approving review (single maintainer — a required reviewer would block all merges); stale-review dismissal, resolved threads, and branch freshness stay enforced.
...
| `crates/signal-fish-client-godot/src/lib.rs` | Godot 4.5 native/web `WebSocketPeer` adapter and its 53 fake-backend tests |
...
`shutdown_timeout` and terminates the loop so parked reliable senders resolve; the polling driver emits synchronously from `poll()` — or a `Transport` method panic (contract violation; only `abort` forbids panics): the async loop task dies, so the channel closes with no `Disconnected`, parked reliable sends resolve `NotConnected`, `abort` still runs, the snapshot stays stale until `shutdown()` reconciles it, and a panicking `abort` during unwind aborts the process; the polling driver propagates the panic into the caller, and `close()` heals — via the close-deadline `abort` only if the violating method keeps panicking.
...
v2 JSON protocol. Server 0.9.1 commit `24a5d10b9e1700cdbef24f05dfe7fe1f0719ac3d` is the released runtime compatibility binding (wire byte-identical to the 0.8.0/0.9.0 releases).
