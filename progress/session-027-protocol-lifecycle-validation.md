...
| --- | --- | --- |
| Authentication, `ProtocolInfo`, room admission/exit, and room-scoped messages are accepted only in their legal phase; v3-only messages require negotiated v3. `Error` remains legal before authentication; `Pong` is connection-scoped even while authentication/negotiation are in flight; and `RelayStats`/`GoingAway` remain connection-scoped after v3 negotiation. | Server 0.7 `src/websocket/connection.rs`; `src/server/message_router.rs:251-400`; `src/server/shutdown.rs:112-155`; AsyncAPI `Pong` at lines 1626-1630 and `PeerTransportStatus`/`RelayStats`/`GoingAway` at lines 2001-2079. | `client_core::tests::lifecycle_classifier_rejects_pre_auth_pre_room_post_room_and_v2_v3_mismatches`; `client_core::tests::pong_is_connection_scoped_before_authentication_and_negotiation`; `client_core::tests::player_room_baseline_requires_the_local_player_exactly_once`; `polling_parity_tests::lifecycle_plan_and_signal_matrix_has_complete_driver_parity`. |
...
| Current-generation inbound signals and every outbound signal target only a non-self peer in both the authoritative WebRTC peer set and current room roster. Replans and departures revoke authority; stale-generation inbound signals remain harmless reordering and are suppressed. Because Server 0.4 omitted generations, a signal after a generation-less relay re-plan or departure is likewise suppressed only when retained peer identity proves prior authorization. | Server 0.7 `src/server/signaling.rs:126-323`; AsyncAPI `Signal` generation at lines 893-908; pinned Server 0.4 generation-omission compatibility. | `client_core::tests::signal_peer_membership_is_shared_by_inbound_and_outbound_paths`; `client_core::tests::replacement_plan_and_player_left_remove_signal_authority`; `client_core::tests::stale_signal_after_relay_replan_is_silently_suppressed`; `client_core::tests::generationless_signal_after_relay_replan_is_silently_suppressed`; `client_core::tests::generationless_departure_order_preserves_the_stale_signal_fence`; `client::tests::reliable_signal_revalidates_peer_departure_while_waiting_for_capacity`; `polling_parity_tests::unauthorized_outbound_signals_fail_without_wire_output_in_both_drivers`. |
