...
/// their engine (tungstenite fuses on protocol errors) or surface the
/// corruption for the driver to treat as terminal (the Godot adapter
/// reports invalid UTF-8 text packets as a receive error, which ends the
/// stream at the driver); byte-oriented backends decode raw text payloads
/// themselves and must use this helper for the same guarantee. The
