---
name: markdown-and-doc-validation
description: Parse and validate project Markdown without corrupting fenced content. Use when editing docs tooling, MkDocs navigation, code fences, admonitions, generated Markdown, or documentation consistency checks.
---
...
When extracting `.md` filenames from `mkdocs.yml`, filter out YAML comments (`grep -vE '^[[:space:]]*#'`) **before** running extraction (`grep -oE`). Extraction discards the leading `#`, making a downstream comment filter a no-op. See `../ci-configuration/SKILL.md` "Pipeline ordering for context-dependent filters" for the general rule. Reference: `scripts/validate-docs.sh` uses the correct ordering.
...
Scripts that validate documentation (e.g., `check-docs-rendering.sh`, `validate-docs.sh`) must use portable regex. See the "Portable regex" section in `../ci-configuration/SKILL.md` for the full rules. The `test_shell_portability.sh` script and `ci_config_tests.rs` Rust tests enforce this.
...
conversion's observable **contract**, not its mechanism (`via unwrap_or(Null)` rots) — see [serde-patterns](../serde-patterns/SKILL.md).
